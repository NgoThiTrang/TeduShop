...
namespace TeduShop.Web.API
{
    ...
    public class ApplicationUserController : ApiControllerBase
    {
        ...
        public ApplicationUserController(IErrorService errorService, IApplicationRoleService appRoleService, IApplicationGroupService appGroupService, ApplicationUserManager userManager) : base(errorService)
        {
      ...
      //  [Authorize(Roles = "ViewUser")]
        public async Task<HttpResponseMessage> Details(HttpRequestMessage request, string id)
        {
            ...
            {
                return request.CreateResponse(HttpStatusCode.BadRequest, nameof(id) + "not found");
            }
            var user =  await _userManager.FindByIdAsync(id); // await là đợi cho hàm này chạy xopng r mới chạy tiếp
            if (user == null) // nếu k để await mà dể như nó thì duới e phai user.Result để đảm bảo ham này n chay ra kết quả do bất đồng bộ nên phải làm thế
            {
                return request.CreateResponse(HttpStatusCode.NoContent, "khong co nguoi dung nay");
            }
            else
            {
                var applicationUserViewModel = Mapper.Map<ApplicationUser, ApplicationUserViewModel>(user); // viết như nay cũng được ợ
                ...
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        foreach (var group in applicationUserViewModel.Groups)
                        {
                            ...
                            {
                                await _userManager.RemoveFromRoleAsync(appUser.Id, role.Id);
                                await _userManager.AddToRoleAsync(appUser.Id, role.Id);
                            }
                        }
